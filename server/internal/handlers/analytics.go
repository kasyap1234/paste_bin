...
//	@Description	Increment the view count for a specific paste
//	@Tags			analytics
//	@Accept			json
//	@Produce		json
//	@Param			pasteID	query		string			true	"Paste ID"
//	@Success		200		{object}	map[string]string	"View count incremented successfully"
//	@Failure		400		{object}	map[string]string	"Invalid paste ID"
//	@Failure		500		{object}	map[string]string	"Unable to increment view count"
//	@Security		BearerAuth
//	@Router			/analytics/increment-view-count [get]
func (h *AnalyticsHandler) IncrementViewCount(c echo.Context) error {
	pasteIDStr := c.QueryParam("pasteID")
	if pasteIDStr == "" {
		return utils.SendError(c, http.StatusBadRequest, "paste id is required")
	}
	...
	ctx := c.Request().Context()
	err = h.analyticsSvc.IncrementViews(ctx, pasteID)
	if err != nil {
...
// GetAnalyticsByURL godoc
//
//	@Summary		Get analytics by URL
//	@Description	Retrieve analytics for a specific URL
//	@Tags			analytics
//	@Accept			json
//	@Produce		json
...
//	@Failure		500		{object}	map[string]string	"Unable to get analytics"
//	@Security		BearerAuth
//	@Router			/analytics/url [get]
func (h *AnalyticsHandler) GetAnalyticsByURL(c echo.Context) error {
	url := c.QueryParam("url")
	if url == "" {
		return utils.SendError(c, http.StatusBadRequest, "url is required")
	}
	ctx := c.Request().Context()
	analytic, err := h.analyticsSvc.GetAnalyticsByURL(ctx, url)
	if err != nil {
		return utils.SendError(c, http.StatusInternalServerError, "failed to retrieve analytics by url")
	}
	return utils.SendSuccess(c, http.StatusOK, analytic, "analytics retrieved successfully")
}
